...
Then, we collect information on several different common policy responses from [The Oxford COVID-19 Government Response Tracker (OxCGRT)](https://www.bsg.ox.ac.uk/research/research-projects/covid-19-government-response-tracker). In this data, we focus on the date when governments implemented national restrictions on internal movement between cities/regions. And we calculate the increase rate of ten days before and after the date of lockdown to investigate if lockdown could control the increase of cases of covid-19. The two-way ANOVA model is built as below:
$$Y_{ijk} = \mu_{\cdot\cdot} + \alpha_i+\beta_j +\epsilon_{ijk}$$, where the number of observations in each cell is k = 1,..., 10, the lockdown status are i = 0 (no lockdown), 1 (lockdown), and j's denote the 106 country codes. The constraints are (i)$\sum\alpha_i=0$, (ii)$\sum\beta_j=0$, (iii)$\epsilon_{ijk}$ follows $N(0,\sigma^2)$.
The model assumptions are that the residuals follow a normal distribution and have equal variances.

Before we conduct further analysis, I need to test if these assumptions hold based on this data set. I shall check if the interaction effect exists or not and will examine if other assumptions of the ANOVA model violate.
```{r,include=FALSE}
fit5<-aov(rate~status+country_code+status:country_code,data=data4)
anova(fit3,fit5)
```
I use F test to check for the significance of the interaction term. The null hypothesis is $H_0: (\alpha\beta)_{ij}=0$ and the alternative hypothesis is $H_1$: not all $(\alpha\beta)_{ij}$ are zero. The result of F-test is shown as below:
|Res.Df |   RSS Df| Sum of Sq |     F|Pr(>F)|
| ----------- | ----------- |----------- | ----------- |-------- |
 | 1823| 142.07   | | |                
 | 1728| 134.56 95 |   7.5105 |1.0153| 0.4418    
From the table , we find with the significant level $\theta=0.05$,  the p-value is larger than $\theta$, which indicates I accept the null hypothesis that there is no joint effect of the status of lockdown and countries at the significant level 0.05 and our ANOVA model without interaction is reasonable.

After I build the ANOVA model, it is natural to investigate if there is significant association between increase rates of different status of lockdown. Therefore, the null hypothesis is $H_0:\alpha_0=\alpha_1=0$ and the alternative hypothesis is $H_a$:not all $\alpha$s are zero. And we use F test to check if the null hypothesis holds. The result shows the p-value is $7.92\times 10^{-7}$ which means with the significant level $\theta=0.05$ and the normal distribution assumption,  I reject the null hypothesis that all the $\alpha_i$ are equal and there are differences in increase rate before and after lockdown at the significant level 0.05.
```{r}
